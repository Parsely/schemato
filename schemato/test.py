...
class TestSchemato(unittest.TestCase):
    ...
    def test_html_parse(self):
        ...
        for res in results:
            if res.classname == "ParselyPageValidator":
                self.assertTrue(
                    "ttle - invalid parsely-page field" in
    ...
    def test_news_distiller(self):
        expected = {
            'author': None,
            'description': 'An executive at Foxconn, Apple\'s manufacturing '
                           'partner, says the iPhone 5 is "the most difficult '
                           'device" it has ever had to make.',
            'id': None,
            'image_url': 'http://rack.0.mshcdn.com/media/ZgkyMDEyLzEyLzA0LzY4L'
                         '2FwcGxlc21hbnVmLmFDeC5qcGcKcAl0aHVtYgkxMjAweDYyNyMKZ'
                         'QlqcGc/c3babd6d/39c/apple-s-manufacturing-partner-ex'
                         'plains-iphone-5-supply-problems-eaf7f9f5b3.jpg',
            'link': 'http://mashable.com/2012/10/17/iphone-5-supply-problems/',
            'pub_date': None,
            'section': None,
            'site': 'Mashable',
            'title': "Apple's Manufacturing Partner Explains iPhone 5 Supply "
                     "Problems"
        }
        mashable = Schemato(
            "http://mashable.com/2012/10/17/iphone-5-supply-problems/")
